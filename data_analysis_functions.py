...
This is so as to reduce the clutter in the main file and isolate the core functionalities of the application in a separate file
'''
...
def display_statistics_visualization(df, cat_columns, num_columns):
    ...
    if len(cat_columns) != 0:
        ...
        for column in selected_cat_columns:
            ...
            fig = go.Figure(data=[go.Bar(x=value_counts.index, y=value_counts, marker_color='#39FF14')])
...
def display_individual_feature_distribution(df, num_columns):
    ...
    if plot_type == 'Histogram':
        fig = px.histogram(df, x=feature, title=f'Histogram of {feature}', color_discrete_sequence=['#39FF14'])

    elif plot_type == 'Scatter Plot':
        fig = px.scatter(df, x=feature, y=feature, title=f'Scatter plot of {feature}', color_discrete_sequence=['#39FF14'])
        ...
        fig = px.box(df, y=feature, title=f'Box plot of {feature}', color_discrete_sequence=['#39FF14'])
...
def display_scatter_plot_of_two_numeric_features(df, num_columns):
    ...
    if len(num_columns) != 0:
        ...
        scatter_fig = px.scatter(df, x=x_feature, y=y_feature, title=f'Scatter Plot: {x_feature} vs {y_feature}', color_discrete_sequence=['#39FF14'])
...
def categorical_variable_analysis(df, cat_columns):
    ...
    if categorical_plot_type == "Bar Chart":
        fig = px.bar(df, x=categorical_feature, title=f"Bar Chart of {categorical_feature}", color_discrete_sequence=['#39FF14'])

    elif categorical_plot_type == "Pie Chart":
        fig = px.pie(df, names=categorical_feature, title=f"Pie Chart of {categorical_feature}", color_discrete_sequence=['#39FF14'])
        ...
        fig = px.bar(df, x=categorical_feature, color=second_categorical_feature, title=f"Stacked Bar Chart of {categorical_feature} by {second_categorical_feature}", color_discrete_sequence=['#39FF14'])
...
def feature_exploration_numerical_variables(df, num_columns):
    ...
    if len(selected_features) < 2:
        ...
        if st.button("Generate Scatter Plot Matrix"):
            scatter_matrix_fig = px.scatter_matrix(df, dimensions=selected_features, title="Scatter Plot Matrix", color_discrete_sequence=['#39FF14'])
            st.plotly_chart(scatter_matrix_fig, use_container_width=True)

        # Pair Plot
        if st.button("Generate Pair Plot"):
            pair_plot_fig = sns.pairplot(df[selected_features], plot_kws={'color': '#39FF14'})
        ...
        if st.button("Generate Correlation Heatmap"):
            ...
            sns.heatmap(correlation_matrix, annot=True, cmap="coolwarm", linewidths=0.5, annot_kws={"color": "#39FF14"})
...
def categorical_numerical_variable_analysis(df, cat_columns, num_columns):
    ...
    # Create a bar chart
    fig = px.bar(group_data, x=categorical_feature_1, y=numerical_feature_1, title=f"{numerical_feature_1} by {categorical_feature_1}", color_discrete_sequence=['#39FF14'])
    st.plotly_chart(fig, use_container_width=True)
