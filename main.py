...
st.set_page_config(page_icon="✨", page_title="AutoML")
...
st.sidebar.title("AutoML: Automated Exploratory Data Analysis and Modeling")
...
selected = option_menu(
    menu_title=None,
    options=['Home', 'Data Exploration', 'Data Preprocessing', 'Pandas Profile', 'Auto ML Modeling'],
    icons=['house-heart', 'bar-chart-fill', 'hammer', 'list-task', 'gear'],
...
# Create a checkbox in the sidebar to choose between the example dataset and uploaded dataset
    ...
    df = function.load_data(file=r"Auto-EDA\AutoEDA\example_dataset\titanic.csv")
...
# my_bar.empty()
            if type == "Regression":
                if st.button("Train the Model"):
                    progress_text = "Training the Model...."
                    my_bar = st.progress(0, text=progress_text)
